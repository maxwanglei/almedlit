...
ARG AL_MEDLIT_RUNTIME_EXTRA
RUN test -n "${AL_MEDLIT_RUNTIME_EXTRA}" \
	&& uv sync --frozen --no-dev --package al-medlit-backend \
		--no-editable --extra "${AL_MEDLIT_RUNTIME_EXTRA}"
...
CMD ["python", "-m", "scripts.start_worker"]
